...
3. デプロイした Web アプリの「通知ルール」で宛先と監視対象スプレッドシートを保存する。フォームが増えたら、同じルールに監視対象を追加する
4. 「トリガー」で間隔を選んで定期チェックを有効にする

通知文は `{{列名}}` でカスタマイズできます。新しく追加した監視対象の既存行は既読になり、それ以降の増分だけが送られます。

宛先、監視ルール、ログは、最初の保存で作られる「GAS Pages 設定」スプレッドシートに保存します。スクリプトプロパティには、そのスプレッドシートの ID だけが入ります。Webhook URL が入るので、このファイルは共有しないでください。
